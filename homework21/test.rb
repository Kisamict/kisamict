...
class AppTest
  ...
  def create_stations(n)
    n.times { |idx| @stations << Station.new("test_station#{idx + 1}") }
  end

  def create_trains(train_class, n)
    n.times { |idx| @trains << train_class.new("test-#{train_class}#{idx + 1}") }
  end
  ...
  def train_wagons_info(train)
    case train.type
    when "PassengerTrain"
      train.each_wagon { |wagon, idx| INFO_PROC.call(idx + 1, wagon.class, "#{wagon.occupied_seats} #{wagon.available_seats}") }
    when "CargoTrain"
      train.each_wagon { |wagon, idx| INFO_PROC.call(idx + 1, wagon.class, "#{wagon.filled_volume} #{wagon.available_volume}") }
    end
  end
end
